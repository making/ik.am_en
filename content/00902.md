---
title: Notes on Installing Tanzu RabbitMQ RPM Package on Rocky Linux
...
updated: 2026-04-26T10:47:16.666Z
---
...
Enter the IP addresses and hostnames of `rmq-1`, `rmq-2`, and `rmq-3` into `/etc/hosts` on all nodes. Below is an example. If you are using predetermined static IP addresses, it is best to create these entries when cloning `rmq-1`.
...
Check the cluster status with the next command. Ensure that all three nodes are displayed.
