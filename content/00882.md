...
You'd want to be able to troubleshoot while using this Builder, right? First, we want to be able to take stack traces and heap dumps with `jcmd`.
...
However, you get an error like the following and cannot copy it. This is because the `tar` command does not exist within the Pod.
...
However, again you get an error like the following and cannot copy it. This is because the `cat` command also does not exist within the Pod.
