---
title: Notes on Installing VMware Tanzu for Postgres 18.2 on Rocky Linux with HA Configuration Using Patroni
tags: ["Tanzu", "PostgreSQL", "Rocky", "Patroni", "Etcd", "HAProxy"]
categories: ["Middleware", "RDBMS", "PostgreSQL", "Patroni"]
summary: This article shows how to install VMware Tanzu for Postgres 18.2 on Rocky Linux 9 running on OrbStack with a Patroni HA setup, covering Etcd and HAProxy configuration and failover verification.
...
all_nodesポート（5002）経由で`SELECT`を実行すると、全ノードにラウンドロビンで接続され、どのノードからもレプリケーションされたデータが読み取れることが確認できます。
