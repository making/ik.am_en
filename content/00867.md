...
This time, we'll learn the basic elements of Spring AI step by step, and ultimately create an immigration advisor app for "[Chronosia](https://github.com/making/chronosia)", a fictional country.
...
Add the following dependency to `pom.xml` to enable HTTP log checking.
...
`RestClient` is used by the OpenAI `ChatClient`.
...
Modify `HelloController` as follows to receive responses as JSON of type `Response`.
...
The request sent to Ollama's API and the response returned are as follows. You can see that instructions for receiving responses in JSON format have been added to the prompt passed with curl.
...
OpenAI's Chat API is stateless, so LLM responses are determined only from the content sent in the request. If you want to use conversation history, you need to manage the conversation on the client side and include all (or summarized) conversation content in the request.
...
We've been using Testcontainers to start PostgreSQL so far, and the database is initialized every time the application starts, so documents are loaded each time.
...
PostgreSQL started with Testcontainers is initialized every time the application starts, so now let's use PostgreSQL where data persists. With `./mvnw spring-boot:run` instead of `./mvnw spring-boot:test-run`, Testcontainers won't start.
...
Next, let's combine `ChatClient` and `VectorStore` so that chat can answer questions about "Chronosia" that the LLM shouldn't know about. This technique is called Retrieval Augmented Generation (RAG).
...
By searching for related documents from the vector database and adding them to the prompt, we got the correct answer to a question about "Chronosia" that it shouldn't know about originally.
...
Let's also try asking in Japanese.
...
We can see that by utilizing "Chronosia" documents, we can get correct answers to unknown questions.
...
Create the following `ChronosiaController` class. Like the previous `HelloController`, it uses `ChatClient` and `QuestionAnswerAdvisor`. Previously, we specified `QuestionAnswerAdvisor` in `defaultAdvisors` when creating `ChatClient`, but this time we specify it with the `advisors(...)` method when calling `ChatClient`. This is because document search is not needed for the initial persona creation call.
...
We can see that using Spring AI makes it easy to create AI applications like this.
