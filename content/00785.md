---
title: Notes on Tracing Tanzu Application Platform's Contour with OpenTelemetry
...
Contour only supports Tracing via OpenTelemetry (OTLP). In this case, we will use [Tempo](https://grafana.com/oss/tempo/) as a Tracing backend that supports the OTLP protocol. We will also use [Grafana](https://grafana.com/oss/grafana/) as the UI for Tempo.
...
Since Tempo does not have a UI, we install Grafana as the UI. Grafana is also installed using helm.
...
Add the following settings to `tap-values.yaml` so that the Contour config file includes the tracing settings. Here, we change Envoy's access log to JSON format and set it to include the `traceparent` field.
