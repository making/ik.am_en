---
title: Deploy a Rust web app on Tanzu Application Platform
tags: ["Kubernetes", "Cartographer", "Tanzu", "TAP", "Tanzu Build Service", "Rust"]
categories: ["Dev", "CaaS", "Kubernetes", "TAP"]
---

I created a container image for a Rust app using the rust buildpack in [this article (Japanese)](/entries/713).

This time, we will make it possible to deploy the application using the rust buildpack on Tanzu Application Platform (TAP).

To be able to use Rust with TAP, you need to add the Rust Buildpack to the ClusterStore and the Rust group to the ClusterBuilder.<br>
...
Add the Rust Buildpack to the environment installed in [this article (Japanese)](/entries/716)

Prepare an overlay that adds ClusterStore and ClusterBuilder to TanzuNetDependencyUpdater as follows.

Have a look at the [Releases](https://github.com/paketo-community/rust/releases) of the rust buildpack and use the latest 0.21.0 at the time of writing.
...
It is easy to check the status of ClusterStore and ClusterBuilder using [`kp`](https://github.com/vmware-tanzu/kpack-cli) CLI.
...
Create a Workload.
Use the app created in [this article (Japanese)](/entries/708).
...
Check the logs with [stern](https://github.com/stern/stern).
